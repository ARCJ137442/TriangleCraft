package TriangleCraft.Structure.Structures
{
	...
	public class Structure_1 implements IStructure
	{
		...
		public function Structure_1(x_:int,y_:int,size:uint=5,seed:int=0):void
		{
			...
			for(var x:int=-size;x<=size;x++)
			{
				for(var y:int=-size;y<=size;y++)
				{
					if(Math.abs(x)+Math.abs(y)==size)
					{
						...
						{
							this.tileGrid.setTileAt(x,y,new Tile(null,null,x,y,TileIDSpace.SPECIAL_WALL.randomID))
						}
						else if(Math.abs(this.x)%Math.abs(this.y)==1)
						{
							this.tileGrid.setTileAt(x,y,new Tile(null,null,x,y,TileID.Ruby_Wall))
						}
						else if(Math.abs(this.x)%Math.abs(this.y)==2)
						{
							this.tileGrid.setTileAt(x,y,new Tile(null,null,x,y,TileID.Emerald_Wall))
						}
						else if(Math.abs(this.x)%Math.abs(this.y)==3)
						{
							this.tileGrid.setTileAt(x,y,new Tile(null,null,x,y,TileID.Sapphire_Wall))
						}
						else
						{
							this.tileGrid.setTileAt(x,y,new Tile(null,null,x,y,TileID.Basic_Wall))
						}
					}
			...
			//Walls Spawner
			this.tileGrid.setTileAt(0,0,new Tile(null,null,0,0,TileID.Walls_Spawner))
			//----====Structure====----//
		}
