package TriangleCraft.Player
{
	...
	public class Player extends Mobile implements IhasSelectableInventory
	{
		...
		protected static const SELECT_ALPHA_DELAY:uint=Game.FPS/2;
		...
		protected var _selectItemRevealed:TileDisplayObj=new TileDisplayObj(TileDisplayFrom.IN_PLAYER,xSize,0,TileID.Void);
		protected var _revealedAlphaDelay:int=Player.SELECT_ALPHA_DELAY;
		...
		{
			var realRadiusX:Number=(xSize-LINE_SIZE)/2
			var realRadiusY:Number=(ySize-LINE_SIZE)/2
			graphics.clear();
			graphics.lineStyle(LINE_SIZE,this.lineColor);
		...
		protected function UpdateRevealedAlpha():void
		{
			...
			{
				if(this._revealedAlphaDelay<=-Player.SELECT_ALPHA_DELAY) this._revealedAlphaDelay=Player.SELECT_ALPHA_DELAY
				else this._revealedAlphaDelay--
				this._selectItemRevealed.alpha=Math.abs(this._revealedAlphaDelay)/Player.SELECT_ALPHA_DELAY
			}
		}
		...
		public function traceSelectedItem(Trace:Boolean=true,ShowItemAttributes:Boolean=true):String
		{
			...
			Str+="Id="+this.selectedItem.id.name+";";
			Str+="Count="+this.selectedItem.count+";";
			if(TileSystem.needColor(selectedItem.id))
			{
		...
		{
			var dir:int=MobileRot.mobileRotToTileRot(this.rot)
		...
		{
			this.host.playerMove(this,MobileRot.ROT_LEFT,this.moveDistence)
		}
		
		public function MoveRight():void
		{
			this.host.playerMove(this,MobileRot.ROT_RIGHT,this.moveDistence)
		}
		
		public function MoveUp():void
		{
			this.host.playerMove(this,MobileRot.ROT_UP,this.moveDistence)
		}
		
		public function MoveDown():void
		{
			this.host.playerMove(this,MobileRot.ROT_DOWN,this.moveDistence)
		}
