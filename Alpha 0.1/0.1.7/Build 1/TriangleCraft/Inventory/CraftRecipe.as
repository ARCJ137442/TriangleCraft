package TriangleCraft.Inventory
{
	...
	public class CraftRecipe
	{
		...
		protected function findItemAsChar(Char,Current:Array):InventoryItem
		{
			...
			if(Loc>=0)
			{
				ItemId=Current[Loc+1] as TileID
				...
				if(!ItemId is TileID||!ItemData is int||!ItemRot is int)
				{
					throwError("Invalid")
				}
		...
		public function getInputID(Loc:uint):TileID
		{
			if(isInNull(Loc))
			{
				return TileID.Void
		...
		public function getOutputID(Loc:uint):TileID
		{
			if(isOutNull(Loc))
			{
				return TileID.Void
