package TriangleCraft.Entity
{
	...
	public class EntityItem extends Entity
	{
							 ...
							 xd:Number=NaN,yd:Number=NaN,vr:Number=NaN):void
		{
			...
			//setItem
			this.item=new InventoryItem(Id,Count,Data)
			//setDisplay
			this.tile=new TileDisplayObj(TileDisplayFrom.IN_ENTITY,0,0,Id,Data,0,Size,Size)
			this.addChild(tile)
		}
		
		//==========SET==========//
		public function setItem(id:String=null,count:uint=0,data:int=0):void
		{
			var runId:String=id,runCount:uint=count,runData:int=data
			if(id==null) runId=this.id
			if(count==0) runCount=this.count
			this.item.setItem(runId,runCount,runData)
			this.tile.changeTile(runId,runData)
		}
		...
		protected function onMove(Ent:Entity=null):void
		{
			...
			//LifeTime & PickupTime
			if(lifeTime<EntityItem.MaxLifeTime) lifeTime++ else this.lifeComplete()
