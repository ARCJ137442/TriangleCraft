package TriangleCraft.Tile
{
	...
	public class TileDisplayObj extends Sprite
	{
		...
		protected var _customShape:TileCustomShape;
									   ...
									   sizeY:uint=TileSystem.globalTileSize):void
		{
			...
			this._customShape=new TileCustomShape()
			this._shape.scaleX=this._xSize/100;
			this._shape.scaleY=this._ySize/100;
		...
		public function changeTile(id:TileID,data:uint=0,rot:int=0,hard:uint=0,maxHard:uint=0):void
		{
			if(TileSystem.isAllowID(id))
			{
				...
				this.maxHard=(maxHard>0)?maxHard:TileAttributes.fromID(this._id).defaultMaxHard;
		...
		public function setDisplay():void
		{
			...
			if(this.attributes!=null&&this.attributes.technical&&this._displayFrom==TileDisplayFrom.IN_ENTITY)
			{
				...
				if(this._customShape.visible)
				{
					this._customShape.removeCurrent()
					this._customShape.visible=false
				}
			...
			//Custom
			else if(TileCustomShape.hasCurrent(this._id,this._data))
			{
				this._customShape.x=-this._xSize/2
				this._customShape.y=-this._ySize/2
				if(this._displayFrom!=TileDisplayFrom.IN_ENTITY)
				{
					this._customShape.scaleX=this._customShape.scaleY=2
				}
				else
				{
					this._customShape.scaleX=this._customShape.scaleY=1
				}
				this._shape.visible=false
				this._customShape.setCurrent(this._id,this._data)
				this._customShape.visible=true
				clearColor()
			}
			//Other
			else
