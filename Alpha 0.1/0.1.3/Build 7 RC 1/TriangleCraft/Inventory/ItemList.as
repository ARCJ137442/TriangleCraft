package TriangleCraft.Inventory
{
	...
	public class ItemList
	{
		...
		public function canHasItemIn(Id:String,Data:int,Tag:TileTag,Rot:int,
									 IgnoreData:Boolean=false,
									 IgnoreTag:Boolean=true,
									 IgnoreRot:Boolean=true)
		{
			return !strictByFilter||canHasInvItemIn(new InventoryItem(Id,1,Data,Tag,Rot))
		}
										...
										IgnoreRot:Boolean=true)
		{
			...
			return this._itemFilter.some(function(item2:InventoryItem,index:uint,vector:Vector.<InventoryItem>)
										 {
											 return item2.isEqual(item,true,IgnoreData,IgnoreTag,IgnoreRot)
										 })
		}
		...
		{
			//Detect
			if(!TileSystem.isAllowID(Item.Id)||!this.canHasInvItemIn(Item)) return
			//Real add
		...
		public function giveItemto(IL:ItemList,id:String,count:uint=1,data:int=0,tag:TileTag=null,rot:int=0):void
		{
			giveInvItemto(IL,new InventoryItem(id,count,data,tag,rot))
		}
