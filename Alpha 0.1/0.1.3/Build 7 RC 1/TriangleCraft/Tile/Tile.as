package TriangleCraft.Tile
{
	...
	public class Tile extends TileDisplayObj implements IhasInventory
	{
		...
		intc function initDropItem():void
		{
			...
			{
				this.DropItems=new ItemList(new InventoryItem(this.ID,1,this.Data,this.Tag,this.Rot))
			}
			else
			{
				...
				while(true)
				{
					replaceTag=replaceID==null?null:TileSystem.getTagFromID(replaceID)
					if(replaceID==null||!TileSystem.isAllowID(replaceID)) break
					...
					{
						this.DropItems=new ItemList(new InventoryItem(replaceID,1,this.Data,this.Tag,this.Rot))
						break
					}
