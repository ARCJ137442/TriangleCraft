package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const VERSION:String="Alpha 0.1.3 build 4";
		...
		intc var WidthByBlock:uint=DISPLAY_WIDTH/TILE_SIZE;
		intc var HeightByBlock:uint=DISPLAY_HEIGHT/TILE_SIZE;
		...
		private function initDefaultRecipes():void
		{
			...
			addCraftRecipe("wbw","bcb","wbw",["w",TileID.Basic_Wall,0,null,0,
											  "b",TileID.Colored_Block,0x888888,null,0,
		...
		{
			return Game.TILE_SIZE*this.ZoomScaleX
		}
		
		private function get TileDisplaySizeY():Number
		{
			return Game.TILE_SIZE*this.ZoomScaleY
		}
		...
		{
			var item:Item=new Item(this,X*TILE_SIZE,
								   Y*TILE_SIZE,
		...
		private function DropItem(X:int,Y:int):void
		{
			...
			if(blockDropItems!=null)
			{
				for(var i:uint=0;i<blockDropItems.typeCount;i++)
				{
					...
					for(var j:uint=0;j<giveCount;j++)
					{
						...
						{
							summonItem(X,Y,giveId,giveCount,0,giveTag,giveRot)
						}
						else
						{
							summonItem(X,Y,giveId,giveCount,giveData,giveTag,giveRot);
						}
					}
				}
			}
			...
			{
				summonItem(X,Y,blockID,1,blockData,blockTag,0);
			}
		}
		...
		{
			var tile=new Tile(this,X*TILE_SIZE,Y*TILE_SIZE,Id,Data,Tag,Rot,Level);
		...
		intc function cloneTile(x1:int,y1:int,x2:int,y2:int,Mode:String="Replace",Hardness:uint=0):void
		{
			...
			//Clone
			if(hasSpellInString("keep",Mode)&&!tTag.canPlaceBy) return
			if(hasSpellInString("move",Mode)) setVoid(x1,y1)
			if(hasSpellInString("swap",Mode)&&!tTag.canPlaceBy) setTile(x1,y1,tId,tData,tTag,tRot,tHard)
			if(hasSpellInString("destroy",Mode)) DropItem(x2,y2)
			setTile(x2,y2,oId,oData,oTag,oRot,oHard)
			//Clone Inventory
			if(oBlock.Tag.hasInventory) tInventory.resetToInventory(oInventory)
			if(hasSpellInString("swap",Mode)&&tBlock.Tag.hasInventory) oInventory.resetToInventory(tInventory)
			//Clone DropItems
											...
											oldNearbyId:String,oldNearbyData:int,oldNearbyTag:TileTag,oldNearbyRot:int,oldNearbyHard:uint):void
		{
			...
			switch(Id)
			{
				...
				case TileID.Signal_Wire:
					...
					var linkCount:uint=uint(hasLinkUp)+uint(hasLinkDown)+uint(hasLinkLeft)+uint(hasLinkRight)
						   ...
						   !detectEntity(X+Xl,Y+Yl))
						{
							if(getTileTag(X+Xl,Y+Yl)!=null)
							{
							 	   ...
							 	   getTileID(X+Xl,Y+Yl)==TileID.XX_Virus)//&&getTileData(X+Xl,Y+Yl)>0
								{
									...
									{
										spawnRandomColoredBlock(X+Xl,Y+Yl)
										return
									}
		...
		private function colorMixerMixColor(X:int,Y:int):void
		{
			...
			var AverageColorH:Number=General.getAverage2(BlockColorsH.filter(function(N:Number,i:uint,V:Vector.<Number>):Boolean{return !isNaN(N)},null));
							...
							for(Xp=-1;Xp<=1;Xp++)
							{
								...
								if(TileSystem.isAllowID(returnId))
								{
									...
									if(CR.returnAsItem)
									{
										...
										{
											summonItem(X+0.5,Y,returnId,returnCount,returnData,
		...
		private function inventoryBlockTransPortItem(X:int,Y:int,IL:IhasInventory,Mode:String):void
		{
			...
			switch(Mode.toLowerCase())
			{
				...
				if(giveItem!=null)
				{
					...
					summonItem(X,Y+0.5,giveItem.Id,1,giveItem.Data,
		...
		intc function hasLink(X:int,Y:int,rot:uint=TileRot.RIGHT):Boolean
		{
			...
			return isTile(vx,vy)?General.binaryToBooleans(getTileTag(vx,vy).canBeLink,4)[(rot-getTileRot(vx,vy)+6)%4]:false
		}
		...
		intc function patchSignal(x:int,y:int,memory:Vector.<Point>=null):void
		{
			...
			{
				trace("mem:",hasInMemory(memoryBlocks,p))
				//Filter
				if(!isTile(p.x,p.y)||hasInMemory(memoryBlocks,p)) continue patchLoop
				//Set In Memory
				...
				switch(block.ID)
				{
					...
					case TileID.Signal_Wire:
						...
						break;
					case TileID.Signal_Patcher:
						patchSignal(x+1,y,memoryBlocks)
						patchSignal(x-1,y,memoryBlocks)
						patchSignal(x,y+1,memoryBlocks)
						patchSignal(x,y-1,memoryBlocks)
						break;
					case TileID.Wireless_Signal_Transmitter:
						patchWirelessSignal(x,y,block.Rot,memoryBlocks)
						break;
					//Use-Signal Machine
					case TileID.Color_Mixer:
		...
		intc function patchWirelessSignal(x:int,y:int,rot:uint,memory:Vector.<Point>=null):void
		{
			...
			var memoryBlocks:Vector.<Point>=memory==null?new Vector.<Point>:memory
			trace("patchWirelessSignal:",vx,vy,cx,cy,getTileRot(cx,cy),(rot+2)%4)
			for(var i:uint=0;i<maxWirelessSignalTransmissionDistance;i++)
			{
				...
				if(!getTileTag(cx,cy).canPass)
				{
					...
					{
						memoryBlocks.push(new Point(cx,cy))
						if(getTileRot(cx,cy)==(rot+2)%4)//Opposite
						{
							patchSignal(cx+1,cy,memoryBlocks)
							patchSignal(cx-1,cy,memoryBlocks)
							patchSignal(cx,cy+1,memoryBlocks)
							patchSignal(cx,cy-1,memoryBlocks)
							break
						}
					}
		...
		private static function hasSpellInString(spell:String,string:String):Boolean
		{
