		...
		public function Structure_2(x_:int,y_:int,size:uint=5,seed:int=0):void
		{
			...
			for(var x:int=-size;x<=size;x++)
			{
				for(var y:int=-size;y<=size;y++)
				{
					...
					{
						this.tileGrid.setTileAt(x,y,new Tile(null,x,y,TileID.Colored_Block,0x000000))
					}
					else if((Math.abs(x)==size-1)!=(Math.abs(y)==size-1)||(Math.abs(x)==size-1)&&(Math.abs(y)==size-1))
					{
						this.tileGrid.setTileAt(x,y,new Tile(null,x,y,TileID.Colored_Block,0x888888))
					}
			...
			this.tileGrid.setTileAt(ms,ms,new Tile(null,ms,ms,TileID.Block_Spawner))
			this.tileGrid.setTileAt(-ms,ms,new Tile(null,-ms,ms,TileID.Block_Spawner))
			this.tileGrid.setTileAt(-ms,-ms,new Tile(null,-ms,-ms,TileID.Block_Spawner))
			this.tileGrid.setTileAt(ms,-ms,new Tile(null,ms,-ms,TileID.Block_Spawner))
			//----====Structure====----//
