package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const VERSION:String="Alpha 0.1.3 build 3";
		...
		private var Structures:Vector.<IStructure>=new Vector.<IStructure>
		...
		private function initDefaultRecipes():void
		{
			...
			addCraftRecipe("vbv","bbb","vbv",["b",TileID.Colored_Block,0x000000,null,0],
						   "lll","lvl","lll",["l",TileID.Link_Line,0,null,0])
		}
		...
		intc function detectStructure(x:int,y:int):IStructure
		{
			...
			if(Structure_1_Condition)
			{
				...
				//trace("Structure_1:",x+","+y,",size="+Structure_1_Size,(Math.abs((x-y)*(y-x))+Math.abs(V1*V3+V4*V2)),(Math.abs(x*y)+Math.abs(y*V1+V2*V3+2048)),Math.abs(x*y)%V1)
				return new Structure_1(x,y,Structure_1_Size,this.WorldSpawnSeed)
			}
			//Structure_2:Colored Blocks With Some Block Spawner
			var Structure_2_Condition=((Math.abs((x+y)*(y+x))+Math.abs(V1*V2+V3*V4))%(Math.abs(x*y)+Math.abs(y*V1+V2*V3+2048))==Math.abs(x*y)%V1)
			if(Structure_2_Condition)
			{
				var Structure_2_Size=2+(Math.abs(x+y)+V1+V2+V3)%4
		...
		private function DropItem(X:int,Y:int):void
		{
			...
			if(blockDropItems!=null)
			{
				for(var i:uint=0;i<blockDropItems.typeCount;i++)
				{
					...
					{
						spawnItem(X,Y,giveId,giveCount,0,giveTag,giveRot)
					}
					else
					{
		...
		intc function cloneTile(x1:int,y1:int,x2:int,y2:int,Mode:String="Replace",Hardness:uint=0):void
		{
			...
			if(Mode.toLowerCase().indexOf("swap")>=0&&!tTag.canPlaceBy) setTile(x1,y1,tId,tData,tTag,tRot,tHard)
											...
											oldNearbyId:String,oldNearbyData:int,oldNearbyTag:TileTag,oldNearbyRot:int,oldNearbyHard:uint):void
		{
			...
			switch(Id)
			{
				...
				case TileID.Link_Line:
					var hasLinkUp:Boolean=isTile(X,Y-1)?getTileTag(X,Y-1).canBeLink:false
					var hasLinkDown:Boolean=isTile(X,Y+1)?getTileTag(X,Y+1).canBeLink:false
					var hasLinkLeft:Boolean=isTile(X-1,Y)?getTileTag(X-1,Y).canBeLink:false
					var hasLinkRight:Boolean=isTile(X+1,Y)?getTileTag(X+1,Y).canBeLink:false
					var linkCount:uint=uint(hasLinkUp)+uint(hasLinkDown)+uint(hasLinkLeft)+uint(hasLinkRight)
					//trace(TileID.Link_Line,linkCount,hasLinkUp,uint(hasLinkUp))
					...
					switch(linkCount)
					{
						...
						case 2:
							if(hasLinkUp!=hasLinkDown)
							{
								...
								if(hasLinkUp)
								{
									shouldRot=hasLinkRight?TileRot.RIGHT:TileRot.UP
								}
								else if(hasLinkDown)
								{
									shouldRot=hasLinkRight?TileRot.DOWN:TileRot.LEFT
								}
							}
							else
							{
		...
		{
			//trace(arguments)
			//spawnItem(X*TILE_SIZE,Y*TILE_SIZE,Id,1,Data,Tag,Rot)
		}
		...
		//======================================//
		//Color Mixer
		private function colorMixerMixColor(X:int,Y:int)
		{
