package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.1.2 build 2";
		...
		intc function spawnPlayer(ID:uint,X:int=0,Y:int=0,color:Number=NaN):Player
		{
			...
			player.gameMode=PlayerGameMode.CREATIVE
					...
					if(isTile(frontX,frontY))
					{
						if(blockTag.canDestroy||blockID!=TileID.Void&&player.Ability.canDestroyAll)
						{
							...
							else
							{
								...
								if(!blockTag.canPlaceBy) Place=false;
								//Set Hook
		...
		intc function detectItem(item:Item):void
		{
		...
		intc function getTileX(tile:Tile):int
		{
			return tile.x/tileSize;
		}

		intc function getTileY(tile:Tile):int
		{
		...
		intc function UpdateDebugText(textNum:int=0):void
		{
			if(textNum<0)
			{
				switch(textNum)
