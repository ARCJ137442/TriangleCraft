package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const VERSION:String="Alpha 0.1.5 build 4";
		...
		//================Game Load Function================//
		//==================================================//
		public function Game(Variables:GameVariables=null,rules:GameRules=null):void
		{
		...
		intc function setVoid(X:int,Y:int):void
		{
			if(isTile(X,Y))
			{
				setTile(X,Y,TileID.Void,0,TileTag.getTagFromID(TileID.Void),0,0)
			}
		}
											...
											oldNearbyId:String,oldNearbyData:int,oldNearbyTag:TileTag,oldNearbyRot:int,oldNearbyHard:uint):void
		{
			...
			switch(Id)
			{
				...
				case TileID.Signal_Diode:
					...
					tile.Data=shouldData
					break;
				//Signal Decelerator:Link Blocks And Change Color
				case TileID.Signal_Decelerator:
					hasLinkFront=hasLink(X,Y,Rot-TileRot.RIGHT)
					hasLinkBack=hasLink(X,Y,Rot-TileRot.LEFT)
					linkCount=uint(hasLinkFront)+uint(hasLinkBack)
					shouldData=Data>3&&getTileTag(X,Y).specialTags.hasTag("delay")&&uint(getTileTag(X,Y).specialTags.getTagValue("delay"))>0?4:0
					shouldData+=(linkCount!=0?(linkCount==1?(hasLinkFront?1:2):3):0)
					tile.Data=shouldData
					break;
		...
		protected function onTickTileRun(information:TickRunTileInformations):void
		{
			...
			switch(block.ID)
			{
				case TileID.Signal_Decelerator:
					...
					if(value>0) sTag.setTagValue("delay",value-1)
					else
					{
						...
						setTile(x,y,TileID.Signal_Decelerator,getTileData(x,y)-4,getTileTag(x,y),getTileRot(x,y))
		...
		protected function signalLampTrunLight(x:int,y:int,data:int=-1):void
		{
			...
			else if(data==-1)//Signal Trigger
			{
				...
				if(sTag!=null)
				{
					if(!sTag.hasTag("lastActiveColor")||sTag.getTagValue("lastActiveColor",uint)==0)
					{
						sTag.setTagValue("lastActiveColor",5)
					}
					tempData=tileData==0?sTag.getTagValue("lastActiveColor",uint):0
				}
				else
				{
			...
			{
				sTag.setTagValue("lastActiveColor",tempData)
			}
		...
		{
			if(!isTile(x,y)) return
			var isInvent:Boolean=getTileData(x,y)>3
			var data:int=getTileData(x,y)
		...
		protected function hasByteSignal(x:int,y:int):Boolean
		{
			...
			if(getTileTag(x,y).hasInventory)
			{
				return true
			}
			return false
		...
		protected function signalByteCopyerCopyByte(x:int,y:int,memory:Vector.<Point>=null):void
		{
			...
			var bX:int=x-TileRot.toPos(rot)[0],bY:int=y-TileRot.toPos(rot)[1]
			var fX:int=x+TileRot.toPos(rot)[0],fY:int=y+TileRot.toPos(rot)[1]
			if(!isTile(fX,fY)||!isTile(bX,bY)) return
			if(hasByteSignal(bX,bY)&&getTileData(x,y)%4==3) setByte(fX,fY,getByte(bX,bY,getTileData(x,y)>3))//Only Active On Full Link
		}
		...
		intc function patchASignal(x:int,y:int,p:Point,memoryBlocks:Vector.<Point>):void
		{
			...
			switch(block.ID)
			{
				...
				case TileID.Signal_Decelerator:
					if(new Point(p.x-x,p.y-y).equals(TileRot.toPosPoint(getTileRot(p.x,p.y))))
					{
						setTile(p.x,p.y,TileID.Signal_Decelerator,getTileData(p.x,p.y)+4,getTileTag(p.x,p.y),getTileRot(p.x,p.y),0,false)
						getTileTag(p.x,p.y).specialTags.addNewTag("delay",10,uint)
