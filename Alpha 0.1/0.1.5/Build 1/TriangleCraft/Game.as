package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const VERSION:String="Alpha 0.1.5 build 1";
		...
		private function getRandomGenerator(x:int,y:int):RandomGenerator
		{
		...
		private function initDefaultRecipes():void
		{
			...
			//Signal Decelerator
			addCraftRecipe("www","waw","www",["w",TileID.Signal_Wire,0,null,0,
											  "a",TileID.Signal_Diode,0,null,0],
						   "vvv","vsv","vvv",["s",TileID.Signal_Decelerator,0,null,0],
						   true,true,true,true,true)
			//Signal Byte Getter
		...
		private function getEntityOn(X:int,Y:int):Entity
		{
											...
											oldNearbyId:String,oldNearbyData:int,oldNearbyTag:TileTag,oldNearbyRot:int,oldNearbyHard:uint):void
		{
			...
			switch(Id)
			{
				...
				case TileID.Signal_Decelerator:
					...
					tile.Data=shouldData
					break;
				//Signal Byte Getter|Setter:Link Blocks
				case TileID.Signal_Byte_Getter:
					hasLinkBack=hasByteLink(X,Y,Rot-TileRot.LEFT,false)
					shouldData=(Data>1?2:0)+(hasLinkBack?1:0)
					tile.Data=shouldData
					break
				case TileID.Signal_Byte_Setter:
					hasLinkFront=hasByteLink(X,Y,Rot-TileRot.RIGHT,true)
		...
		private function onPlayerUseBlock(P:Player,X:int,Y:int,Pos:uint,Id:String,Data:int,Tag:TileTag,Rot:int,Hard:uint):void
		{
			...
			switch(Id)
			{
				...
				case TileID.Signal_Byte_Storage:
					...
					break
				//Signal Byte Getter|Setter:Trun Mode
				case TileID.Signal_Byte_Getter:
				case TileID.Signal_Byte_Setter:
		...
		private function onRandomTick(X:int,Y:int,Id:String,Data:int,Tag:TileTag,Rot:int,Hard:uint):void
		{
			...
			switch(Id)
			{
				...
				for(i=0;i<20;i++)
				{
					...
					hasEntityOn(cx,cy)||getTileTag(cx,cy)==null) break
				...
				for(xd=-2;xd<=2;xd++)
				{
					for(yd=-2;yd<=2;yd++)
					{
						...
						//Transform
						else if(Math.abs(xd)>1||Math.abs(yd)>1)
						{
		...
		private function crystalWallActive(X:int,Y:int):void 
		{
		...
		private function signalLampTrunLight(x:int,y:int,data:int=-1):void
		{
			...
			if(data==-1)
			{
				...
				{
					case 0://Off
					tempData=tcMath.randomBetween(1,6)
					break
					default:
					tempData=0
					break
		...
		//Block Pusher
		private function blockPusherPushBlock(x:int,y:int):void
		{
			//Define
		...
		//Block Puller
		private function blockPullerPullBlock(x:int,y:int):void
		{
			//Define
			var rot:int=getTileRot(x,y)
			var vx:int=TileRot.toPosPoint(rot).x
			...
			for(i=firstI;i<lastI&&i>1;i++)
			{
				...
				if(!isTile(lx,ly)||
				!getTileTag(lx,ly).canBeMove) break
				cloneTile(cx,cy,lx,ly,"move")
			}
		}
		
		//Signal Byte Storage
		private function signalByteStorageTrunByte(x:int,y:int,mode:int=-1):void
		{
		...
		private function signalByteGetterOrSetterTrunMode(x:int,y:int):void 
		{
			if(!isTile(x,y)) return
			var isInvent:Boolean=getTileData(x,y)>1
			var data:int=getTileData(x,y)
			getTileObject(x,y).Data+=isInvent?-2:2
		}
		
		private function signalByteGetterGetByte(x:int,y:int,memory:Vector.<Point>=null):void
		{
			var rot:int=getTileRot(x,y)
			var bX:int=x-TileRot.toPos(rot)[0],bY:int=y-TileRot.toPos(rot)[1]
			if(!isTile(bX,bY)) return
			var byte:Boolean=false,reverse:Boolean=getTileData(x,y)>1
			var tileID:String=getTileID(bX,bY),tileData:int=getTileData(bX,bY)
			switch(tileID)
			{
				case TileID.Signal_Byte_Storage:
				case TileID.Signal_Lamp:
				...
				case TileID.Colored_Block:
					...
					break
				case TileID.Inventory_Block:
					byte=getTileObject(bX,bY).AllItemCount>0
					break
			}
			if(reverse)byte=!byte
			if(byte)patchSignal(x,y,memory)
		}
		
		private function signalByteSetterSetByte(x:int,y:int,memory:Vector.<Point>=null):void
		{
			var rot:int=getTileRot(x,y)
			var fX:int=x+TileRot.toPos(rot)[0],fY:int=y+TileRot.toPos(rot)[1]
			if(!isTile(fX,fY)) return
			var reverse:Boolean=getTileData(x,y)>1
			var tileID:String=getTileID(fX,fY),tileData:int=getTileData(fX,fY)
			var shouldData:int=tileData
			switch(tileID)
			{
				case TileID.Signal_Byte_Storage:
					shouldData=reverse?0:1
					break
				case TileID.Signal_Lamp:
					shouldData=reverse?0:1+random(5)
					break
				case TileID.Colored_Block:
					shouldData=reverse?(tileData<0xffffff/2?0xffffff-shouldData:shouldData):(tileData>0xffffff/2?0xffffff-shouldData:shouldData)
					break
			}
			if(shouldData!=tileData) setTile(fX,fY,tileID,shouldData)
		}
		
		//Signal Transmitters:
		private function hasLink(x:int,y:int,rot:uint=TileRot.RIGHT):Boolean
		{
		...
		private function hasByteLink(x:int,y:int,rot:uint=TileRot.RIGHT,isSetter:Boolean=false):Boolean
		{
			...
			return (tileID==TileID.Signal_Byte_Storage||
					...
					!isSetter&&tileID==TileID.Inventory_Block)
		}
		
		private function getLinkedPoses(x:int,y:int):Vector.<Point>
		{
		...
		private function getLinkedBlocks(x:int,y:int):Vector.<Tile>
		{
			var returnVec:Vector.<Tile>=new Vector.<Tile>
			if(isVoid(x,y)) return returnVec
			//Check Link
		...
		intc function patchSignal(x:int,y:int,memory:Vector.<Point>=null):void
		{
			...
			patchLoop:for each(var p:Point in linkedPoses)
			{
				...
				switch(block.ID)
				{
					...
					case TileID.Signal_Byte_Getter:
						if(!new Point(p.x-x,p.y-y).equals(TileRot.toPosPoint(getTileRot(p.x,p.y)))&&
						   !new Point(p.x-x,p.y-y).equals(TileRot.toPosPoint(TileRot.toOpposite(getTileRot(p.x,p.y)))))
						{
							signalByteGetterGetByte(p.x,p.y,memoryBlocks)
						}
						break
					case TileID.Signal_Byte_Setter:
						if(!new Point(p.x-x,p.y-y).equals(TileRot.toPosPoint(TileRot.toOpposite(getTileRot(p.x,p.y)))))
						{
					...
					//Use-Signal Blocks
					case TileID.Crystal_Wall:
						if(General.randomBoolean(1,2))
						crystalWallActive(p.x,p.y)
						break
					case TileID.Color_Mixer:
					...
					case TileID.Block_Puller:
						...
						break
					case TileID.Signal_Byte_Storage:
						signalByteStorageTrunByte(p.x,p.y)
						break
				}
			}
		}
