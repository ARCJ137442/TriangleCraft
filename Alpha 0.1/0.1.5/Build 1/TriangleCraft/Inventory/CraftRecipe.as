package TriangleCraft.Inventory
{
	...
	public class CraftRecipe
	{
		...
		public function testCanCraft(Inputs:Vector.<InventoryItem>):Boolean
		{
			...
			{
				if(!this._inputItems[i].isEqual(Inputs[i],
							this.ignoreCount,
							this.ignoreData,
							this.ignoreTag||TileTag.getTagFromID(Inputs[i].Id).ignoreDataInCraftRecipe,
							this.ignoreRot||TileTag.getTagFromID(Inputs[i].Id).ignoreRotInCraftRecipe))
				return false
			}
			return true
		}
		
		public function getCraftPercent(Inputs:Vector.<InventoryItem>,includeVoid:Boolean=true):Number
		{
			return getCraftEquals(Inputs,includeVoid)/ITEM_SLOT_COUNT
		}
		
		public function getCraftEquals(Inputs:Vector.<InventoryItem>,includeVoid:Boolean=true):uint
		{
			...
			for(var i:uint=0;i<Inputs.length;i++)
			{
				...
				if(item.isEqual(Inputs[i],this.ignoreCount,this.ignoreData,this.ignoreTag,this.ignoreRot))
				equals++
			}
			return equals
		}
