package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.1.1 build 2";
		...
		//Game Rule
		private static const VirusModes:Array=["Default","VirusStrom"]
		...
		//Game Rule
		private var VirusMode:String="Default"
		...
		private function setWorldVariables():void
		{
			...
			//Game Rules
			VirusMode=String(VirusModes[random(VirusModes.length)])
			switch(VirusMode)
			{
				case "VirusStrom":
				MaxVirusCount=10
		...
		private function setWorldSpawnConditions(MaxCount:uint):void
		{
			this.E1=General.IsiA(1,WorldSpawnConditions)
			this.E2=General.IsiA(2,WorldSpawnConditions)
			this.E3=General.IsiA(3,WorldSpawnConditions)
			this.E4=General.IsiA(4,WorldSpawnConditions)
			this.E5=General.IsiA(5,WorldSpawnConditions)
		}
		...
		private function getTileBySeed(X:int=0,Y:int=0):InventoryItem
		{
			...
			Condition_4=E4&&(General.isPrime(Math.abs(X*Y+F1+F2-V4))&&General.isPrime(Math.abs(X*Y-F1-F2+V4)))
