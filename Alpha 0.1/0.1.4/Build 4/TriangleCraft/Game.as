package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const VERSION:String="Alpha 0.1.4 build 4";
		...
		intc function detectCraftRecipePreview(crp:CraftRecipePreview):void
		{
			...
			//Set Recipe
			var silmerNums:Vector.<uint>=new Vector.<uint>,tempNum:uint=0
			var item:InventoryItem,item2:InventoryItem
			for(var i:uint=0;i<this.Craft_Recipes.length;i++)
			{
				tempNum=0
				var recipe:CraftRecipe=this.Craft_Recipes[i]
				if(recipe==null)
				{
					silmerNums.push(0)
					continue
				}
				for(var loc:uint=0;loc<recipe.inputItemCount;loc++)
				{
					item=recipe.getInputItem(loc)
					item2=loc<inputItems.length?inputItems[loc]:null
					if(item.Id!=TileID.Void&&item2!=null&&
					   inputItems[loc].isEqual(item,recipe.ignoreCount,
					   recipe.ignoreData,recipe.ignoreTag,recipe.ignoreRot))
					{
						tempNum++
					}
				}
				silmerNums.push(tempNum)
			}
			for each(var n:uint in silmerNums)
			{
				if(n>tempNum) tempNum=n
			}
			//Final Set
			crp.setRecipe(tempNum==0?null:this.Craft_Recipes[silmerNums.indexOf(tempNum)],inputItems)
		}
