package TriangleCraft
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.1.0 build 3";
		...
		private function onGameLoaded(E:Event=null):void
		{
			...
			y=middleDisplayY;
			//Spawn Player
			var P1=SpawnPlayer(PlayerCount+1,0,0);
			//Spawn Tile
			spawnTiles()
			//Set DebugText
		...
		private function loadDefaultRecipes():void
		{
			...
			addCraftRecipe("crc","bvg","cyc",["c",TileSystem.Colored_Block,0x888888,null,0,
						   ...
						   "vav","ava","vav",["a",TileSystem.Color_Mixer,0,null,0])
			//Block Crafter
			//G G G
			//R G B
			//G G G
			addCraftRecipe("ggg","rgb","ggg",["g",TileSystem.Colored_Block,0x888888,null,0,
											  "r",TileSystem.Colored_Block,0xff0000,null,0,
											  "b",TileSystem.Colored_Block,0x0000ff,null,0],
						   "vvv","vav","vvv",["a",TileSystem.Block_Crafter,0,null,0])
			//Bacic Wall
			//B B B
			//B G B
			//B B B
			addCraftRecipe("bbb","bgb","bbb",["b",TileSystem.Colored_Block,0x000000,null,0,
						   ...
						   "gag","aaa","gag",["a",TileSystem.Basic_Wall,0,null,0,
											  "g",TileSystem.Colored_Block,0x888888,null,0])
			//Block Spawner
			//B G B
			//G G G
			...
			addCraftRecipe("bgb","ggg","bgb",["b",TileSystem.Colored_Block,0x000000,null,0,
											  "g",TileSystem.Colored_Block,0x888888,null,0],
						   "vvv","vav","vvv",["a",TileSystem.Block_Spawner,0,null,0])
			//Arrow Block
			//B B
			//G G B
			//B B
			addCraftRecipe("bbv","ggb","bbv",["b",TileSystem.Colored_Block,0x000000,null,0,
											  "g",TileSystem.Colored_Block,0x888888,null,0],
						   "vvv","vav","vvv",["a",TileSystem.Arrow_Block,0,null,0])
			//Barrier
			//W W W
			//W W W
			//W W W
			addCraftRecipe("www","www","www",["w",TileSystem.Basic_Wall,0,null,0],
						   "vvv","vbv","vvv",["b",TileSystem.Barrier,0,null,0])
			//Crystal Wall
			//C C C
			//C W C
			//C C C
			addCraftRecipe("ccc","cwc","ccc",["w",TileSystem.Basic_Wall,0,null,0,
											  "c",TileSystem.Colored_Block,0x00ffff,null,0],
						   "vwv","vvv","vwv",["w",TileSystem.Crystal_Wall,0,null,0])
			//Walls Spawner
			//B W B
			//W S W
			//B W B
			addCraftRecipe("bwb","wsw","bwb",["b",TileSystem.Colored_Block,0x000000,null,0,
											  "w",TileSystem.Basic_Wall,0,null,0,
											  "s",TileSystem.Block_Spawner,0,null,0],
						   "vvv","vav","vvv",["a",TileSystem.Walls_Spawner,0,null,0])
			//XX Virus
			addCraftRecipe("bgb","gsg","bgb",["b",TileSystem.Colored_Block,0x000000,null,0,
											  "g",TileSystem.Colored_Block,0x888888,null,0,
											  "s",TileSystem.Block_Spawner,0,null,0],
						   "vvv","vvv","vvv",["v",TileSystem.XX_Virus,0,null,0])
			//XX Virus Red
			addCraftRecipe("vvv","vrv","vvv",["r",TileSystem.Colored_Block,0xff0000,null,0,
											  "v",TileSystem.XX_Virus,0,null,0],
						   "vvv","vvv","vvv",["v",TileSystem.XX_Virus_Red,0,null,0])
			//XX Virus Blue
			addCraftRecipe("vvv","vbv","vvv",["b",TileSystem.Colored_Block,0x0000ff,null,0,
											  "v",TileSystem.XX_Virus,0,null,0],
						   "vvv","vvv","vvv",["v",TileSystem.XX_Virus_Blue,0,null,0])
		}
		...
		{
			//World Seed
			WorldSpawnSeed=random(int.MAX_VALUE);
		...
		private function getRandomSpawnConditions(MaxCount:uint):Array
		{
			...
			if(General.isNullArray(returnArr))
			{
				returnArr=[1,2,3,4,5]
			}
			return returnArr;
		...
		private function getTileBySeed(X:int=0,Y:int=0):InventoryItem
		{
			...
			var Condition_1:Boolean=General.IsiA(1,WorldSpawnConditions)&&(X*V2+(V1*V2)%F2==Math.abs(Y-X)%V3)
			var Condition_2:Boolean=General.IsiA(2,WorldSpawnConditions)&&(Y*V4+(V1*V2)%F2==Math.abs(X-Y)%V3)
			var Condition_3:Boolean=General.IsiA(3,WorldSpawnConditions)&&(Math.floor(Math.abs(X+V5)+Math.abs(Y-V5))%Math.abs(V4*X-V2*Y-X*Y*V5)==V5%V4)
			var Condition_4:Boolean=General.IsiA(4,WorldSpawnConditions)&&(General.isPrime(Math.abs(X+Y+F1+F2-V4))&&General.isPrime(Math.abs(X+Y-F1-F2+V4)))
			var Condition_5:Boolean=General.IsiA(5,WorldSpawnConditions)&&(Math.abs(V5*X+F1)%16==Math.abs(V5*Y+F1)%32)
		...
		{
			if(ID>0&&PlayerCount>0)
			{
				var P=getPx(PlayerCount);
				if(P is Entity)
				{
					var Px=SpawnPlayer(PlayerCount+1,P.getX()+random(2)*2-1,P.getY()+random(2)*2-1);
