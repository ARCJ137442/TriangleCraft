package 
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.0.3 build 4";
		...
		private function OnKeyDown(E:KeyboardEvent):void
		{
			...
			if(E.ctrlKey&&E.shiftKey&&Code>48&&Code<58&&Code-49==getPlayerCount()&&Code-48<=LimitPlayerCount)
			{
				spawnPx((Code-48));
				return;
			}
			//Set Pos To Player
			if(!E.ctrlKey&&E.shiftKey&&Code>48&&Code<58&&Code-48<=getPlayerCount())
			{
		...
		private function PlayerUse(player:MovieClip,Distance:uint):void
		{
			...
			var blockDropItems=block.DropItems
			...
			if(player.GameMode.CanDestroy&&Destroy)
			{
				if(block!=null)
				{
					...
					if(block.Tag.canDestroy)
					{
						...
						{
							//Clean Item
							if(player.InventorySelect<0&&block.Tag.canGet||
							player.InventorySelect>-1&&!block.Tag.canPlaceBy)
							{
								...
								{
									player.AddItem(blockID,1,blockData,blockTag,0);
								}
							...
							setTile(frontX,frontY,Tile.Void,0);
							//Set Hook
		...
		//--------Tile Functions--------//
		private function setNewTile(X:int=0,Y:int=0,Id:String=Tile.Void,Data:int=0,Tag:TileTag=null):Object
		{
		...
		private function getTileID(X:int,Y:int):String
		{
			if(isTile(X,Y))
			{
		...
		private function getTileTag(X:int,Y:int):TileTag
		{
			if(isTile(X,Y))
			{
		...
		private function setTile(X:int,Y:int,Id:String,Data:int=0,Tag:TileTag=null,Rot:int=0):Boolean
		{
			var tile=getTileObject(X,Y);
			if(tile!=null)
			{
		...
		private function cloneTile(x1:int,y1:int,x2:int,y2:int,Mode:String="Replace"):void
		{
			...
			if(Mode.indexOf("Move")>=0) setTile(x1,y1,Tile.Void,0,null)
			setTile(x2,y2,oId,oData,null/*oTag*/)
		}

		//--------Hook Functions--------//
		private function onBlockDestroy(X:int,Y:int,P:MovieClip,Id:String,Data:int,Tag:TileTag,Rot:int):void
		{
			//trace(arguments)
		}

		private function onBlockPlace(X:int,Y:int,P:MovieClip,Id:String,Data:int,Tag:TileTag,Rot:int):void
		{
			//trace(arguments)
		}
							...
							if(TestCanCraft(SlotId,NeedSlotId,SlotData,NeedSlotData)==true)
							{
								for(Yp=-1;Yp<=1;Yp++)
								{
									for(Xp=-1;Xp<=1;Xp++)
									{
										...
										//Clean
										setTile(X+offsetX+Xp,Y+offsetY+Yp,Tile.Void,0);
										if(OutputId[l]==null)
										{
											continue;
										}
		...
		private function onRandomTick(X:int,Y:int,Id:String,Data:int,Tag:TileTag,Rot:int):void
		{
			...
			switch(Id)
			{
				...
				for(var C:uint=0;C<9;C++)
				{
					...
					{
						var tempHard:uint=getTileHard(X,Y)
						cloneTile(X,Y,cx,cy,"Move")
						setTileHard(X,Y,1)
						setTileHard(cx,cy,tempHard)
						return
					}
					else
					{
