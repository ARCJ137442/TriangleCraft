package 
{
	...
	public class Game extends Sprite
	{
		...
		static const Version:String="Alpha 0.0.1 build 2";
		//Tile System
		...
		private function getTileBySeed(X:int=0,Y:int=0):Array
		{
			...
			var Conditions:Boolean=X*V2+(V1*V2)%V4==WorldSpawnSeed%Math.abs((Y-F1))||Y*V4+(V1*V2)%V4==WorldSpawnSeed%Math.abs((X-F2))||Math.floor(Math.abs((X+V5))+Math.abs((Y-V5)))%Math.abs((((V4*X)-V2*Y)-X*Y*V5))==V5%V4||isPrime(Math.abs((((X+Y)+F1)+F2))-V4)&&isPrime(Math.abs((((X+Y)-F1)-F2))+V4)||Math.abs(((V5*X)+F1))%16==Math.abs(((V5*Y)+F1))%32;
		...
		private function OnKeyUp(E:KeyboardEvent):void
		{
			...
			if ((getPlayerCount()>0))
			{
				for (var i=0; i<getPlayerCount(); i++)
				{
					...
					if (! player.ActiveKey_Up&&! player.ActiveKey_Down&&! player.ActiveKey_Left&&! player.ContolKey_Right&&! player.ContolKey_Use&&! player.ContolKey_Select_Left&&! player.ContolKey_Select_Right)
					{
		...
		private function PlayerMove(player:MovieClip,Rot:uint,Distance:uint):void
		{
			...
			{
				//Border Test
				if (((((((rX<BorderWidth)&&rX>0)&&Rot==3)||rX>displayWidth-BorderWidth&&rX<displayWidth&&Rot==1)||rY<BorderHeight&&rY>0&&Rot==4)||rY>displayHeight-BorderHeight&&rY<displayHeight&&Rot==2))
				{
		...
		private function PlayerUse(player:MovieClip,Distance:uint):void
		{
			...
			if (player.GameMode.CanDestroy&&Destroy)
			{
				if ((block!=null))
				{
					...
					{
						//Clean Item
						if (player.InventorySelect<0&&block.TileTag.canGet||player.InventorySelect>-1&&! block.TileTag.canPlaceBy)
						{
		...
		private function onBlockUse(X:int,Y:int,Rot:uint,Id:int,Data:int,Tag:Object):void
		{
			...
			if ((Id==2))
			{
				...
				{
					BlockColorsR.push(Tile.trunRGB(BlockColors[c])[0])
					BlockColorsG.push(Tile.trunRGB(BlockColors[c])[1])
					BlockColorsB.push(Tile.trunRGB(BlockColors[c])[2])
				}
				...
				var AverageColor:uint=Player.RGBToCombination([AverageColorR,AverageColorG,AverageColorB])
				var i;
				for (i in MixBlocks)
				{
					var t=MixBlocks[i];
