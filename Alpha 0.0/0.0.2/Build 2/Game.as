package 
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.0.2 build 2";
		...
		private var MaxVirusCount:uint=3
		...
		private function getTileBySeed(X:int=0,Y:int=0):Array
		{
			...
			if (testEntity(X,Y)==null&&Conditions)
			{
				...
				if((V1+V2+V3+V4+F1+F2+F3)%512==(V1+V2+V3+V4)
				   &&VirusCount<MaxVirusCount)
				{
		...
		private function EnterFrame(E:Event)
		{
			...
			//Random Tick
			if(randomTickCount>0)
			{
				for(var d=0;d<randomTickCount;d++)
				{
					var ranX=(random(2)*2-1)*random(WorldWidth+1)
					var ranY=(random(2)*2-1)*random(WorldWidth+1)
					onRandomTick(ranX,ranY,getTileID(ranX,ranY),getTileData(ranX,ranY),getTileTag(ranX,ranY))
				}
			}
			//Reset Timer
			LastTime=getTimer();
		...
		private function cloneTile(x1:int,y1:int,x2:int,y2:int,Mode:String="Replace"):void
		{
			...
			if(Mode.indexOf("Move")>0)
			{
				setTile(x1,y1,0,0,null)
			}
		...
		private function onRandomTick(X:int,Y:int,Id:int,Data:int,Tag:Object):void
		{
			...
			//{
				switch(Id)
				{
					//Block Spawner
					case 5:
					for(var Xl:int=-1;Xl<=1;Xl++)
					{
						for(var Yl:int=-1;Yl<=1;Yl++)
						{
					...
					case 6:
					//Influence
					for(var c:uint=random(3)+1;c>0;c--)
					{
						//Repair Self
