package 
{
	...
	public class Game extends Sprite
	{
		...
		public static const Version:String="Alpha 0.0.2 build 4";
		...
		private function setWorldVariables():void
		{
			...
			//About World Spawn
			V1=Number(String(WorldSpawnSeed).charAt(WorldSpawnSeed%String(WorldSpawnSeed).length)+WorldSpawnSeed%10);//1~99
		...
		private function PlayerMove(player:MovieClip,Rot:uint,Distance:uint):void
		{
			...
			else if (getTileID(mX,mY)>0)
			{
				...
				{
					onBlockUse(mX,mY,player,Rot,getTileID(mX,mY),getTileData(mX,mY),getTileTag(mX,mY));
				}
			}
		}
				...
				if (block!=null)
				{
					...
					if (block.TileTag.canDestroy)
					{
						...
						else
						{
							...
							player.InventorySelect>-1&&!block.TileTag.canPlaceBy)
							{
								...
								if(block.TileDropItems is Array&&block.TileDropItems.length>0)
								{
									for(var i in block.TileDropItems)
									{
										...
										var giveTag:Object=copyObject(blockTag)
										player.AddItem(giveId,giveCount,giveData,giveTag);
									}
								}
								else
								{
							...
							//Set Hook
							onBlockDestroy(frontX,frontY,player,blockID,blockData,blockTag);
							//Set Variable
							Place=false;
						}
					}
			...
			if (player.InventorySelect>-1&&player.GameMode.CanPlace&&Place)
			{
				...
				if (testEntity(frontX,frontY)==null&&block!=null)
				{
					...
					block.TileTag.canPlaceBy)
					{
						...
						UpdateDebugText(2);
						//Set Hook
						onBlockPlace(frontX,frontY,player,PlaceId,PlaceData,PlaceTag);
					}
				}
			}
		}
		...
		private function getTile(X:int,Y:int):Array
		{
			if (isTile(X,Y))
			{
				return [getTileID(X,Y),getTileData(X,Y),getTileTag(X,Y)];
			}
			return null;
		}
		...
		private function setTileHard(X:int,Y:int,Hard:uint):void
		{
			...
			{
				var block=getTileObject(X,Y)
				block.setHardness(Math.round(Math.min(getTileMaxHard(X,Y),Math.max(Hard,0))))
			}
		}
		...
		//--------Hook Functions--------//
		private function onBlockDestroy(X:int,Y:int,P:MovieClip,Id:int,Data:int,Tag:Object):void
		{
			//trace(arguments)
		}

		private function onBlockPlace(X:int,Y:int,P:MovieClip,Id:int,Data:int,Tag:Object):void
		{
			//trace(arguments)
		}

		private function onBlockUse(X:int,Y:int,P:MovieClip,Rot:uint,Id:int,Data:int,Tag:Object):void
		{
			switch (Id)
			{
...
//
		...
		private function onRandomTick(X:int,Y:int,Id:int,Data:int,Tag:Object):void
		{
			/*if(emableRadomTickOutPut)
			{
				trace("Random X:"+X,"Random Y:"+Y,"Arguments:"+arguments)
			}*/
			//if(Tag!=null&&Tag.allowRandomTick)
			//{
				switch(Id)
				{
					//Block Spawner
					...
					for(var Xl:int=-2;Xl<=2;Xl++)
					{
						for(var Yl:int=-2;Yl<=2;Yl++)
						{
							if(Math.abs(Xl)!=Math.abs(Yl)&&
							   getTileID(X+Xl,Y+Yl)>=0&&
							   getTileTag(X+Xl,Y+Yl)["canPlaceBy"])
							{
								if(random(17)&&!detectEntity(X+Xl,Y+Yl))
								{
									var r:uint=5
		...
		//============Code Functions============//
		public static function getPropertyOnObject(arr:Array,pro:String):Array
		{
